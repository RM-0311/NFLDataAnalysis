...
pbp_comp_passes = pd.concat([
    pbp_ksu[pbp_ksu['pass'] == True].reset_index(),
])

print(pbp_comp_passes.head(10))
pbp_comp_passes['passing_yards'] = np.where(pbp_comp_passes['statYardage'] >= 20, "long", "short")
pbp_comp_passes['passing_yards'] = np.where(pbp_comp_passes['statYardage'] < 0, "backward")
pbp_comp_passes["passing_yards"] = np.where(pbp_comp_passes["passing_yards"].isnull(), 0, pbp_comp_passes["passing_yards"])

print(pbp_comp_passes["passing_yards"].describe())

# Convert to HTML table
html_table = pbp_comp_passes.to_html(index=False)  # Exclude row index
